# RELATION-BASED KNOWLEDGE DISTILLATION (RelBKD)
...
# METRICS & CUSTOM TRAINER (Relation-Based KD)
...
class RelBKDTrainer(Trainer):
    """
    Relation-Based KD (RKD-D + RKD-A):
    aligns inter-sample relations between teacher and student embeddings.
    """
    def __init__(self, teacher_model, alpha=0.5, lambda_distance=1.0, lambda_angle=1.0, *args, **kwargs):
    ...
    def pairwise_distances(x):
        ...
        dists = norms + norms.T - 2.0 * torch.matmul(x, x.T)
        dists = torch.sqrt(F.relu(dists) + 1e-8)
        return dists / (dists.mean() + 1e-8)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Handle missing hidden states (e.g. if decoder doesn't return them) 
        # Use encoder hidden states as default for relation matching.
        if hasattr(teacher_outputs, "encoder_last_hidden_state"):
            t_embed = teacher_outputs.encoder_last_hidden_state.mean(dim=1)
        elif hasattr(teacher_outputs, "hidden_states") and teacher_outputs.hidden_states:
            t_embed = teacher_outputs.hidden_states[-1].mean(dim=1)
        else:
            raise AttributeError("Teacher model did not return hidden states; enable via config.")

        if hasattr(student_outputs, "encoder_last_hidden_state"):
            s_embed = student_outputs.encoder_last_hidden_state.mean(dim=1)
        elif hasattr(student_outputs, "hidden_states") and student_outputs.hidden_states:
            s_embed = student_outputs.hidden_states[-1].mean(dim=1)
        else:
            raise AttributeError("Student model did not return hidden states; enable via config.")

        # Compute relational losses 
        t_d = self.pairwise_distances(t_embed)
        s_d = self.pairwise_distances(s_embed)
        t_a = self.pairwise_angles(t_embed)
        s_a = self.pairwise_angles(s_embed)

        loss_dist = F.smooth_l1_loss(s_d, t_d)
        loss_angle = F.smooth_l1_loss(s_a, t_a)
        loss_rkd = self.lambda_distance * loss_dist + self.lambda_angle * loss_angle

        total_loss = self.alpha * loss_rkd + (1 - self.alpha) * loss_ce
...
def make_objective(task_name, teacher_path, train_ds, val_ds):
    def objective(trial):
        ...
        lambda_distance = trial.suggest_categorical("lambda_distance", [0.5, 1.0, 2.0])
        lambda_angle = trial.suggest_categorical("lambda_angle", [0.5, 1.0, 2.0])
...
with open("./optuna_results/best_taskA_RelBKD_params.json", "w") as f:
    json.dump(bestA, f, indent=4)
with open("./optuna_results/best_taskB_RelBKD_params.json", "w") as f:
...
with open("./optuna_results/best_taskA_RelBKD_params.json", "r") as f:
    bestA = json.load(f)
with open("./optuna_results/best_taskB_RelBKD_params.json", "r") as f:
    bestB = json.load(f)
...
def train_with_best(task_name, teacher_path, train_ds, val_ds, params):
    ...
    print(f"\nFull KD Training for {task_name} | α={alpha}, λ_d={lambda_distance}, λ_a={lambda_angle}, lr={lr}, bs={bs}")
    ...
    trainer.train()
    trainer.save_model(f"./studentRelBKD_{task_name}")
    del teacher, student, trainer
